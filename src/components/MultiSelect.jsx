...
// Renders directly inside an already-expanded accordion section body — no
// popover/trigger of its own, so the accordion is the only thing the user
// has to open to reach the checkboxes.
export default function MultiSelect({ groups, options, selected, onChange, searchable = false }) {
  const { t } = useLanguage();
    ...
    <div>
      ...
      <div className="thin-scrollbar max-h-56 overflow-y-auto pr-1">
        {visibleGroups.map((group, gi) => (
          <div key={group.label || gi} className={gi > 0 ? "mt-2" : ""}>
            {group.label && (
              <p className="px-1 py-1 text-[10px] font-semibold uppercase tracking-wide text-neutral-400 dark:text-neutral-500">
                {group.label}
              </p>
            )}
            {group.options.map((option) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className="flex w-full cursor-pointer items-center gap-2 rounded-lg px-1 py-1.5 text-left text-sm text-neutral-700 transition hover:bg-amber-50 dark:text-neutral-200 dark:hover:bg-neutral-800"
                >
                  <span
      ...
      {selected.length > 0 && (
        <button
          type="button"
          onClick={() => onChange([])}
          className="mt-2 w-full cursor-pointer rounded-lg px-1 py-1.5 text-center text-xs text-neutral-500 transition hover:text-amber-700 dark:text-neutral-400 dark:hover:text-amber-400"
        >
