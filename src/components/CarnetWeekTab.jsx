...
function DayColumn({ dayKey, label, stores, onDropStore, onRemoveStore, onSendToAgenda }) {
  const { t } = useLanguage();
      ...
      <div className="thin-scrollbar mb-2 flex min-h-[70px] flex-1 flex-col gap-1.5 overflow-y-auto">
        {stores.length === 0 ? (
          ...
          stores.map((store) => (
            <div
              key={store.id}
              className="flex items-center justify-between gap-1.5 rounded-lg bg-neutral-50 px-2.5 py-1.5 text-xs dark:bg-neutral-800"
            >
...
export default function CarnetWeekTab({ stores, onSendToAgenda }) {
