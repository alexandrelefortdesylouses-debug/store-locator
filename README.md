...
- **26 opticiens rattachés à la mauvaise région** (`scripts/fix_region_geocode_errors.py`, one-off) : la région et le département affichés pour un opticien ne sont pas stockés en base, ils sont recalculés à la volée à partir du code postal présent dans le champ `address` (voir `src/utils/postalCode.js`, `src/utils/regions.js`, `src/utils/departments.js`) — donc une erreur de code postal dans l'adresse source suffit à faire apparaître un opticien dans la mauvaise région, indépendamment de ses coordonnées GPS. Deux causes distinctes identifiées et corrigées :
